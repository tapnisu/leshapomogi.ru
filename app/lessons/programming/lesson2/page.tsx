		...
		<div>
			...
			<div>
				...
				<ul>
					...
					<li>Можно использовать нижнее подчеркивание: deadinside_1000_7</li>
				</ul>
				<h2>Объявление переменных</h2>
				При объявлении переменной нужно указать тип переменной: число, строка и
				т.д. <code>int a;</code> - создаём переменную a с типом int (целое
				число) Так же в 1 строчку можно создавать несколько переменных:
				<code>int a, b, c</code> - создали переменные a, b, c с типом целое
				число (int).
				<h2>Как записать значение в переменную?</h2>
				Для объявления переменной с начальным значением просто укажите значение
				после равно: <code>int a = 993;</code> - создали переменную a со
				значением 993; Так же можно объявлять сразу несколько переменных со
				значениями: <code>int a, b = 1, c = 55;</code> - создали переменные с
				целыми числами: a - без значения, b = 1, c = 55.
...
cout << a; // выведет число 993, так как мы заменили прошлое значение в переменной.`}
				...
				<br />
				(Когда мы заменяем значение переменной мы убираем прошлое значение из
				коробочки, и складываем новое.)
				<ol>
					<li>Оператор – это команда языка программирования (инструкция).</li>
					<li>
						Оператор присваивания – это команда для записи нового значения в
						переменную.
					</li>
				</ol>
				<h2>Ввод данных с клавиатуры</h2>
				<code>{"cin >> a;"}</code> - мы вводим значение с клавиатуры и
				записываем его в переменную a. (Ввод заканчивается при нажатии enter на
				клавиатуре)
				<h2>Ввод значений двух переменных</h2>
				<code>{"cin >> a >> b;"}</code> - если мы введем с клавиатуры{" "}
				<code>25 30</code>, то в переменную a запишется 25, а в b запишется 30.
				<br />
				...
				<code className="language-cpp">setw(5)</code> - set width – установить
				ширину поля (выводит a, с шириной строки 5 - выведет:
				<pre>
					<code>{"    "}5</code>
				</pre>
				...
				<h6>
					Я уже устал печатать, зачем так много, файл уже 140 строчек, за что
					мне это?
				</h6>
				<h2>Типы данных</h2>
				<ol>
